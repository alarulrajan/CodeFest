...
/**
 * Created by IntelliJ IDEA. User: sg897500 Date: Mar 24, 2005 Time: 3:43:17 PM
...
public class TableTag extends HtmlTableTag {
    ...
    private static Log log = LogFactory
            .getLog(org.displaytag.tags.TableTag.class);
    ...
    public void setSort(final String value)
        ...
        if (TableTagParameters.SORT_AMOUNT_PAGE.equals(value)) {
            ...
            throw new InvalidTagAttributeValueException(this.getClass(),
     ...
     */
    public void setDefaultorder(final String value)
            throws InvalidTagAttributeValueException {
    ...
    public void setUid(final String value) {
        if (this.getHtmlId() == null) {
            this.setHtmlId(value); // by default id is actually used for the
                                    // html id attribute, if no htmlId is added
        }
    ...
    public void addColumn(final HeaderCell column) {
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] addColumn " + column);
        }
    ...
    public int doStartTag() throws JspException {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] doStartTag called");
        }

        this.properties = TableProperties
                .getInstance((HttpServletRequest) this.pageContext.getRequest());
        this.tableModel = new TableModel(this.properties, this.pageContext
        ...
        final Object previousMediaType = this.pageContext
                .getAttribute(TableTag.PAGE_ATTRIBUTE_MEDIA);
        ...
        if (this.currentMediaType != null
            ...
            if (TableTag.log.isDebugEnabled()) {
                TableTag.log.debug("[" + this.getUid() + "] setting media ["
                        + this.currentMediaType + "] in this.pageContext");
            }
            this.pageContext.setAttribute(TableTag.PAGE_ATTRIBUTE_MEDIA,
                    this.currentMediaType);
        }

        this.doIteration();

        // always return EVAL_BODY_BUFFERED to get column headers also if the
        // table is empty
    ...
    public int doAfterBody() {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid()
     ...
     * 
     * @return <code>int</code> either EVAL_BODY_BUFFERED or SKIP_BODY depending
     *         on whether another iteration is desired.
     */
    protected int doIteration() {

        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] doIteration called");
        }
        ...
        if (this.tableIterator.hasNext()) {
            ...
            if (this.getUid() != null) {
                if (iteratedObject != null) {
                    ...
                    if (TableTag.log.isDebugEnabled()) {
                        TableTag.log.debug("[" + this.getUid()
                                + "] setting attribute \"" + this.getUid()
                                + "\" in pageContext");
                    }
                    this.pageContext
                            .setAttribute(this.getUid(), iteratedObject);
                ...
                this.pageContext.setAttribute(this.getUid()
                        + TableTagExtraInfo.ROWNUM_SUFFIX, new Integer(
                        this.rowNumber));
            }

            // Row object for Cell values
            this.currentRow = this.setRowObjectForCellValues(iteratedObject,
                    this.rowNumber);
    ...
    private void initParameters() throws ObjectLookupException,
        ...
        final RequestHelper requestHelper = TableTag.rhf
                .getRequestHelperInstance(this.pageContext);

        this.initHref(requestHelper);

        final Integer pageNumberParameter = requestHelper.getIntParameter(this
                .encodeParameter(TableTagParameters.PARAMETER_PAGE));
        this.pageNumber = pageNumberParameter == null ? 1 : pageNumberParameter
                .intValue();

        final Integer sortColumnParameter = requestHelper.getIntParameter(this
                .encodeParameter(TableTagParameters.PARAMETER_SORT));
        final int sortColumn = sortColumnParameter == null ? this.defaultSortedColumn
        ...
        final Integer exportTypeParameter = requestHelper.getIntParameter(this
        ...
        final boolean wishOptimizedIteration = this.pagesize > 0 // we are
                                                                    // paging
        ...
        if (wishOptimizedIteration && this.list instanceof Collection // we need
                                                                        // to
                                                                        // know
                                                                        // the
                                                                        // size
                ...
                ) && (this.currentMediaType == MediaTypeEnum.HTML // and we are
                                                                    // not
                                                                    // exporting
                || !this.properties.getExportFullList())) {
            int start = 0;
            int end = 0;
            if (this.offset > 0) {
                start = this.offset;
     ...
     * 
     * @return <code>true</code> if the current row must be evaluated because is
     *         included in output or because is included in sorting.
     */
    ...
    protected void initHref(final RequestHelper requestHelper) {
        ...
        final Href normalHref = requestHelper.getHref();

        if (this.excludedParams != null) {
            final String[] splittedExcludedParams = StringUtils
            ...
            if (splittedExcludedParams.length == 1
                ...
                final Iterator paramsIterator = normalHref.getParameterMap()
                        .keySet().iterator();
                while (paramsIterator.hasNext()) {
                    final String key = (String) paramsIterator.next();
        ...
        if (this.requestUri != null) {
            ...
            if (!this.dontAppendContext) {
                final String contextPath = ((HttpServletRequest) this.pageContext
            ...
            final Map parameterMap = normalHref.getParameterMap();
    ...
    public int doEndTag() throws JspException {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] doEndTag called");
        }

        if (!this.doAfterBodyExecuted) {
            if (TableTag.log.isDebugEnabled()) {
                TableTag.log
                        .debug("[" + this.getUid() + "] tag body is empty.");
            }
        ...
        final TableDecorator tableDecorator = DecoratorFactory
                .loadTableDecorator(this.decoratorName);
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] doEndTag - end");
        }
        ...
        if (this.tableIterator.hasNext()) {
            ...
            if (iteratedObject instanceof Map) {
                ...
                try {
                    ...
                    TableTag.log.warn("Unable to automatically add columns: "
                            + e.getMessage(), e);
                }
            }

            // iterator on properties names
            final Iterator propertiesIterator = objectProperties.keySet()
                    .iterator();

            while (propertiesIterator.hasNext()) {
                // get the property name
                final String propertyName = (String) propertiesIterator.next();
    ...
    protected int doExport() throws JspException {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] currentMediaType="
                    + this.currentMediaType);
        }

        final boolean exportHeader = this.properties
                .getExportHeader(this.currentMediaType);
        final boolean exportDecorated = this.properties.getExportDecorated();

        final ExportView exportView = ExportViewFactory.getInstance().getView(
    ...
    protected void writeExport(final ExportView exportView) throws IOException,
        ...
        final String filename = this.properties
                .getExportFileName(this.currentMediaType);
    ...
    protected void setupViewableData() {
        ...
        if (this.pagesize > 0) {
            ...
            if (this.pageNumber == -1) {
                fullList = CollectionUtil.getListFromObject(originalData, 0,
                        ((List) originalData).size());
            } else {
    ...
    private void writeHTMLData() throws JspException {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid()
                    + "] getHTMLData called for table [" + this.getUid() + "]");
        }

        final boolean noItems = this.tableModel.getRowListPage().size() == 0;
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] getHTMLData end");
        }
    }
    ...
    private void writeTableHeader() {
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] getTableHeader called");
        }
        ...
        final Iterator iterator = this.tableModel.getHeaderCellList()
                .iterator();
        ...
        while (iterator.hasNext()) {
            ...
            if (headerCell.getSortable()) {
                ...
                final Anchor anchor = new Anchor(
                        this.getSortingHref(headerCell), header);
        ...
        if (TableTag.log.isDebugEnabled()) {
            TableTag.log.debug("[" + this.getUid() + "] getTableHeader end");
        }
    }
     ...
     */
    private Href getSortingHref(final HeaderCell headerCell) {
        // costruct Href from base href, preserving parameters
        final Href href = new Href(this.baseHref);

        // add column number as link parameter
        href.addParameter(
                this.encodeParameter(TableTagParameters.PARAMETER_SORT),
                headerCell.getColumnNumber());

        final boolean nowOrderAscending = !(headerCell.isAlreadySorted() && this.tableModel
                .isSortOrderAscending());

        final int sortOrderParam = nowOrderAscending ? SortOrderEnum.ASCENDING
                .getCode() : SortOrderEnum.DESCENDING.getCode();
        href.addParameter(
                this.encodeParameter(TableTagParameters.PARAMETER_ORDER),
                sortOrderParam);

        // If user want to sort the full table I need to reset the page number.
        if (this.tableModel.isSortFullTable()) {
            href.addParameter(
                    this.encodeParameter(TableTagParameters.PARAMETER_PAGE), 1);
        }
     ...
     * groups the column and returns the appropriate string back to the caller.
     * 
     * @param value
     *            String
     * @param group
     ...
     * @return String
     */
    private String groupColumns(final String value, final int group) {
        ...
        if (this.previousRow.containsKey(new Integer(group))) {
            for (int j = 1; j <= group; j++) {
                ...
                if (!((String) this.previousRow.get(new Integer(j)))
                        .equals(this.nextRow.get(new Integer(j)))) {
     ...
     * 
     * @throws ObjectLookupException
     *             for errors in looking up properties in objects
     * @throws DecoratorException
     *             for errors returned by decorators
     */
    private void writeTableBody() throws ObjectLookupException,
        ...
        final RowIterator rowIterator = this.tableModel.getRowIterator(false);
        ...
        while (rowIterator.hasNext()) {
            ...
            if (this.tableModel.getTableDecorator() != null) {
                final String stringStartRow = this.tableModel
                        .getTableDecorator().startRow();
            ...
            if (TableTag.log.isDebugEnabled()) {
                TableTag.log.debug("[" + this.getUid()
                        + "] creating ColumnIterator on "
                        + this.tableModel.getHeaderCellList());
            }
            final ColumnIterator columnIterator = row
                    .getColumnIterator(this.tableModel.getHeaderCellList());
            ...
            if (this.tableModel.getTableDecorator() != null) {
                final String endRow = this.tableModel.getTableDecorator()
                        .finishRow();
                if (endRow != null) {
        ...
        if (this.tableModel.getRowListPage().size() == 0) {
            this.write(MessageFormat.format(this.properties
                    .getEmptyListRowMessage(), new Object[] { new Integer(
                    this.tableModel.getNumberOfColumns()) }), out);
        }
    }
    ...
    private void writeExportLinks() {
        ...
        final Href exportHref = new Href(this.baseHref);

        final StringBuffer buffer = new StringBuffer(200);
        final Iterator iterator = MediaTypeEnum.iterator();

        while (iterator.hasNext()) {
            final MediaTypeEnum currentExportType = (MediaTypeEnum) iterator
                    .next();
            ...
            if (this.properties.getAddExport(currentExportType)) {
                ...
                exportHref
                        .addParameter(
                                this.encodeParameter(TableTagParameters.PARAMETER_EXPORTTYPE),
                ...
                final Anchor anchor = new Anchor(exportHref,
                        this.properties.getExportLabel(currentExportType));
                buffer.append(anchor.toString());
            }
        }

        final String[] exportOptions = { buffer.toString() };
        this.write(MessageFormat.format(this.properties.getExportBanner(),
                exportOptions));
    }
