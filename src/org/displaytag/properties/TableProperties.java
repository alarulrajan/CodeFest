...
/**
 * Created by IntelliJ IDEA. User: sg897500 Date: Mar 24, 2005 Time: 3:37:57 PM
 * To change this template use File | Settings | File Templates. Taken from: The
 * properties used by the Table tags. The properties are loaded in the following
 * order, in increasing order of priority. The locale of getInstance() is used
 * to determine the locale of the property file to use; if the key required does
 * not exist in the specified file, the key will be loaded from a more general
 * property file.
 * <ol>
 * <li>First, from the TableTag.properties included with the DisplayTag
 * distribution.</li>
 * <li>Then, from the file displaytag.properties, if it is present; these
 * properties are intended to be set by the user for sitewide application.
 * Messages are gathered according to the Locale of the property file.</li>
 * <li>Finally, if this class has a userProperties defined, all of the
 * properties from that Properties object are copied in as well.</li>
 * </ol>
 * 
...
public final class TableProperties implements Cloneable {
	 ...
	 * classpath. Settings in this file will override the defaults loaded from
	 * TableTag.properties.
	 */
	...
	/**
	 * property <code>css.tr.even</code>: holds the name of the css class for
	 * even rows. Defaults to <code>even</code>.
	 */
	public static final String PROPERTY_CSS_TR_EVEN = "css.tr.even"; //$NON-NLS-1$

	/**
	 * property <code>css.tr.odd</code>: holds the name of the css class for odd
	 * rows. Defaults to <code>odd</code>.
	 */
	public static final String PROPERTY_CSS_TR_ODD = "css.tr.odd"; //$NON-NLS-1$

	/**
	 * property <code>css.table</code>: holds the name of the css class added to
	 * the main table tag. By default no css class is added.
	 */
	public static final String PROPERTY_CSS_TABLE = "css.table"; //$NON-NLS-1$

	/**
	 * property <code>css.th.sortable</code>: holds the name of the css class
	 * added to the the header of a sortable column. By default no css class is
	 * added.
	 */
	public static final String PROPERTY_CSS_TH_SORTABLE = "css.th.sortable"; //$NON-NLS-1$

	/**
	 * property <code>css.th.sorted</code>: holds the name of the css class
	 * added to the the header of a sorted column. Defaults to
	 * <code>sorted</code>.
	 */
	public static final String PROPERTY_CSS_TH_SORTED = "css.th.sorted"; //$NON-NLS-1$

	/**
	 * property <code>css.th.ascending</code>: holds the name of the css class
	 * added to the the header of a column sorted in ascending order. Defaults
	 * to <code>order1</code>.
	 */
	public static final String PROPERTY_CSS_TH_SORTED_ASCENDING = "css.th.ascending"; //$NON-NLS-1$

	/**
	 * property <code>css.th.descending</code>: holds the name of the css class
	 * added to the the header of a column sorted in descending order. Defaults
	 * to <code>order2</code>.
	 */
	public static final String PROPERTY_CSS_TH_SORTED_DESCENDING = "css.th.descending"; //$NON-NLS-1$

	/**
	 * prefix used for all the properties related to export ("export"). The full
	 * property name is <code>export.</code> <em>[export type]</em>
	 * <code>.</code><em>[property name]</em>
	 */
	public static final String PROPERTY_EXPORT_PREFIX = "export"; //$NON-NLS-1$

	/**
	 * property <code>export.types</code>: holds the list of export available
	 * export types.
	 */
	...
	/**
	 * The userProperties are local, non-default properties; these settings
	 * override the defaults from displaytag.properties and TableTag.properties.
	 */
	private static Properties userProperties = new Properties();

	/**
	 * Configured resource provider. If no ResourceProvider is configured, an
	 * no-op one is used. This instance is initialized at first use and shared.
	 */
	...
	/**
	 * TableProperties for each locale are loaded as needed, and cloned for
	 * public usage.
	 */
	...
	/**
	 * Setter for I18nResourceProvider. A resource provider is usually set using
	 * displaytag properties, this accessor is needed for tests.
	 * 
	 * @param provider
	 *            I18nResourceProvider instance
	 */
	protected static void setResourceProvider(
			final I18nResourceProvider provider) {
		TableProperties.resourceProvider = provider;
	}

	/**
	 * Setter for LocaleResolver. A locale resolver is usually set using
	 * displaytag properties, this accessor is needed for tests.
	 * 
	 * @param resolver
	...
	private static Properties loadBuiltInProperties()
		...
		try {
			defaultProperties.load(TableProperties.class
					.getResourceAsStream(TableProperties.DEFAULT_FILENAME));
		} catch (final IOException e) {
			throw new TablePropertiesLoadException(TableProperties.class,
					TableProperties.DEFAULT_FILENAME, e);
		}
	...
	/**
	 * Loads user properties (displaytag.properties) according to the given
	 * locale. User properties are not guarantee to exist, so the method can
	 * return <code>null</code> (no exception will be thrown).
	 * 
	 * @param locale
	 *            requested Locale
	 * @return loaded properties
	...
	private static ResourceBundle loadUserProperties(final Locale locale) {
		...
		try {
			bundle = ResourceBundle.getBundle(TableProperties.LOCAL_PROPERTIES,
					locale);
		} catch (final MissingResourceException e) {
			// if no resource bundle is found, try using the context classloader
			try {
				bundle = ResourceBundle.getBundle(
						TableProperties.LOCAL_PROPERTIES, locale, Thread
								.currentThread().getContextClassLoader());
			} catch (final MissingResourceException mre) {
				if (TableProperties.log.isDebugEnabled()) {
					TableProperties.log.debug(Messages.getString(
							"TableProperties.propertiesnotfound", //$NON-NLS-1$
	...
	/**
	 * Returns the configured Locale Resolver. This method is called before the
	 * loading of localized properties.
	 * 
	 * @return LocaleResolver instance.
	 * @throws TablePropertiesLoadException
	 *             if the default <code>TableTag.properties</code> file is not
	 *             found.
	 */
	public static LocaleResolver getLocaleResolverInstance()
		...
		final ResourceBundle defaultUserProperties = TableProperties
				.loadUserProperties(Locale.getDefault());

		// if available, user properties have higher precedence
		if (defaultUserProperties != null) {
			try {
				className = defaultUserProperties
						.getString(TableProperties.PROPERTY_CLASS_LOCALERESOLVER);
			} catch (final MissingResourceException e) {
				// no problem
			}
		}
		...
		if (className == null) {
			...
			className = defaults
					.getProperty(TableProperties.PROPERTY_CLASS_LOCALERESOLVER);
		}

		if (TableProperties.localeResolver == null) {
			if (className != null) {
				try {
					final Class classProperty = ReflectHelper
							.classForName(className);
					TableProperties.localeResolver = (LocaleResolver) classProperty
							.newInstance();

					TableProperties.log
							.info(Messages.getString(
									"TableProperties.classinitializedto", //$NON-NLS-1$
									new Object[] {
											ClassUtils
													.getShortClassName(LocaleResolver.class),
											className }));
				} catch (final Exception e) {
					TableProperties.log
							.warn(Messages.getString(
									"TableProperties.errorloading", //$NON-NLS-1$
									new Object[] {
											ClassUtils
													.getShortClassName(LocaleResolver.class),
											e.getClass().getName(),
				...
				TableProperties.log.info(Messages.getString(
						"TableProperties.noconfigured", //$NON-NLS-1$
						new Object[] { ClassUtils
								.getShortClassName(LocaleResolver.class) }));
			}
			...
			if (TableProperties.localeResolver == null) {
				...
				TableProperties.localeResolver = new LocaleResolver() {
	 ...
	 * the user defined one. There is no caching used here, caching is assumed
	 * to occur in the getInstance factory method.
	 * 
	 * @param myLocale
	...
	private TableProperties(final Locale myLocale)
		...
		final Properties defaultProperties = TableProperties
				.loadBuiltInProperties();

		this.properties = new Properties(defaultProperties);
		this.addProperties(myLocale);

		// Now copy in the user properties (properties file set by calling
		// setUserProperties()).
		// note setUserProperties() MUST BE CALLED before the first
		// TableProperties instantation
		final Enumeration keys = TableProperties.userProperties.keys();
		while (keys.hasMoreElements()) {
			final String key = (String) keys.nextElement();
			if (key != null) {
				this.properties.setProperty(key,
						(String) TableProperties.userProperties.get(key));
			}
		}
	}
	 ...
	 * displaytag.properties, and merge them into the existing properties.
	 * 
	 * @param userLocale
	 *            the locale from which the properties are to be loaded
	 */
	private void addProperties(final Locale userLocale) {
		final ResourceBundle bundle = TableProperties
				.loadUserProperties(userLocale);
	 ...
	 * 
	 * @param request
	 *            HttpServletRequest needed to extract the locale to use. If
	 *            null the default locale will be used.
	 * @return TableProperties instance
	 */
	public static TableProperties getInstance(final HttpServletRequest request) {
		Locale locale;
		if (request != null) {
			locale = TableProperties.getLocaleResolverInstance().resolveLocale(
					request);
		} else {
		...
		TableProperties props = (TableProperties) TableProperties.prototypes
				.get(locale);
		if (props == null) {
	...
	/**
	 * Unload all cached properties. This will not clear properties set by by
	 * setUserProperties; you must clear those manually.
	 */
	...
	/**
	 * Local, non-default properties; these settings override the defaults from
	 * displaytag.properties and TableTag.properties. Please note that the
	 * values are copied in, so that multiple calls with non-overlapping
	 * properties will be merged, not overwritten. Note: setUserProperties()
	...
	public static void setUserProperties(final Properties overrideProperties) {
		// copy keys here, so that this can be invoked more than once from
		// different sources.
		// if default properties are not yet loaded they will be copied in
		// constructor
		final Enumeration keys = overrideProperties.keys();
		while (keys.hasMoreElements()) {
			final String key = (String) keys.nextElement();
			if (key != null) {
				TableProperties.userProperties.setProperty(key,
						(String) overrideProperties.get(key));
			}
		}
	}
	...
	public String getPagingInvalidPage() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_INVALIDPAGE);
	}
	...
	public String getPagingItemName() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_ITEM_NAME);
	}
	...
	public String getPagingItemsName() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_ITEMS_NAME);
	}
	...
	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_FOUND_ONEITEM</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingFoundOneItem() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_FOUND_ONEITEM);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_FOUND_ALLITEMS</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingFoundAllItems() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_FOUND_ALLITEMS);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_FOUND_SOMEITEMS</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingFoundSomeItems() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_FOUND_SOMEITEMS);
	}
	...
	public int getPagingGroupSize() {
		...
		return this.getIntProperty(
				TableProperties.PROPERTY_INT_PAGING_GROUPSIZE, 8);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_BANNER_ONEPAGE</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingBannerOnePage() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_BANNER_ONEPAGE);
	}
	...
	public String getPagingBannerFirst() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_BANNER_FIRST);
	}
	...
	public String getPagingBannerLast() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_BANNER_LAST);
	}
	...
	public String getPagingBannerFull() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_BANNER_FULL);
	}
	...
	public String getPagingPageLink() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_PAGE_LINK);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_PAGE_SELECTED</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingPageSelected() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_PAGE_SELECTED);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_PAGING_PAGE_SPARATOR</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getPagingPageSeparator() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_PAGE_SPARATOR);
	}
	...
	public boolean getExportHeader(final MediaTypeEnum exportType) {
		return this.getBooleanProperty(TableProperties.PROPERTY_EXPORT_PREFIX
				+ TableProperties.SEP + exportType.getName()
				+ TableProperties.SEP
				+ TableProperties.EXPORTPROPERTY_BOOLEAN_EXPORTHEADER);
	}
	...
	public String getExportLabel(final MediaTypeEnum exportType) {
		return this.getProperty(TableProperties.PROPERTY_EXPORT_PREFIX
				+ TableProperties.SEP + exportType.getName()
				+ TableProperties.SEP
				+ TableProperties.EXPORTPROPERTY_STRING_LABEL);
	}
	...
	public String getExportFileName(final MediaTypeEnum exportType) {
		return this.getProperty(TableProperties.PROPERTY_EXPORT_PREFIX
				+ TableProperties.SEP + exportType.getName()
				+ TableProperties.SEP
				+ TableProperties.EXPORTPROPERTY_STRING_FILENAME);
	}

	/**
	 * Getter for the <code>PROPERTY_BOOLEAN_EXPORTDECORATED</code> property.
	 * 
	 * @return boolean <code>true</code> if decorators should be used in
	 *         exporting
	 */
	public boolean getExportDecorated() {
		return this
				.getBooleanProperty(TableProperties.PROPERTY_BOOLEAN_EXPORTDECORATED);
	}
	...
	/**
	 * Getter for the <code>PROPERTY_STRING_EXPORTBANNER_SEPARATOR</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getExportBannerSeparator() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_EXPORTBANNER_SEPARATOR);
	}
	...
	public boolean getShowHeader() {
		return this
				.getBooleanProperty(TableProperties.PROPERTY_BOOLEAN_SHOWHEADER);
	}
	...
	public String getEmptyListMessage() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_EMPTYLIST_MESSAGE);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_EMPTYLISTROW_MESSAGE</code>
	 * property.
	 * 
	 * @return String
	 */
	public String getEmptyListRowMessage() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_EMPTYLISTROW_MESSAGE);
	}

	/**
	 * Getter for the <code>PROPERTY_BOOLEAN_EMPTYLIST_SHOWTABLE</code>
	 * property.
	 * 
	 * @return boolean <code>true</code> if table should be displayed also if no
	 *         items are found
	 */
	public boolean getEmptyListShowTable() {
		return this
				.getBooleanProperty(TableProperties.PROPERTY_BOOLEAN_EMPTYLIST_SHOWTABLE);
	}

	/**
	 * Getter for the <code>PROPERTY_STRING_EXPORTAMOUNT</code> property.
	 * 
	 * @return boolean <code>true</code> if <code>export.amount</code> is
	 *         <code>list</code>
	 */
	 ...
	 * 
	 * @return boolean <code>true</code> if <code>sort.amount</code> is
	 *         <code>list</code>
	 */
	 ...
	 * @param rowNumber
	 *            row number
	 * @return the value of <code>PROPERTY_CSS_TR_EVEN</code> if rowNumber is
	 *         even or <code>PROPERTY_CSS_TR_ODD</code> if rowNumber is odd.
	 */
	public String getCssRow(final int rowNumber) {
		return this
				.getProperty(rowNumber % 2 == 0 ? TableProperties.PROPERTY_CSS_TR_ODD
						: TableProperties.PROPERTY_CSS_TR_EVEN);
	}
	 ...
	 * @return the value of <code>PROPERTY_CSS_TH_SORTED_ASCENDING</code> if
	 *         column is sorded in ascending order or
	 *         <code>PROPERTY_CSS_TH_SORTED_DESCENDING</code> if column is
	 *         sorded in descending order.
	 */
	public String getCssOrder(final boolean ascending) {
		return this
				.getProperty(ascending ? TableProperties.PROPERTY_CSS_TH_SORTED_ASCENDING
	...
	public String[] getExportTypes() {
		final String list = this
				.getProperty(TableProperties.PROPERTY_EXPORTTYPES);
		if (list == null) {
			return new String[0];
		}
	...
	public String getExportClass(final String exportName) {
		return this.getProperty(TableProperties.PROPERTY_EXPORT_PREFIX
				+ TableProperties.SEP + exportName + TableProperties.SEP
				+ TableProperties.EXPORTPROPERTY_STRING_CLASS);
	}
	...
	public RequestHelperFactory getRequestHelperFactoryInstance()
			throws FactoryInstantiationException {
		final Object loadedObject = this
				.getClassPropertyInstance(TableProperties.PROPERTY_CLASS_REQUESTHELPERFACTORY);
		...
		try {
			...
			throw new FactoryInstantiationException(this.getClass(),
					TableProperties.PROPERTY_CLASS_REQUESTHELPERFACTORY,
					loadedObject.getClass().getName(), e);
		}
	}

	/**
	 * Returns the configured resource provider instance. If necessary
	 * instantiate the resource provider from config and then keep a cached
	 * instance.
	 * 
	...
	public I18nResourceProvider geResourceProvider() {
		final String className = this
				.getProperty(TableProperties.PROPERTY_CLASS_LOCALEPROVIDER);
		...
		if (TableProperties.resourceProvider == null) {
			if (className != null) {
				try {
					final Class classProperty = ReflectHelper
							.classForName(className);
					TableProperties.resourceProvider = (I18nResourceProvider) classProperty
							.newInstance();
					...
					TableProperties.log
							.info(Messages.getString(
									...
									new Object[] {
											ClassUtils
													.getShortClassName(I18nResourceProvider.class),
											className }));
				} catch (final Exception e) {
					TableProperties.log
							.warn(Messages.getString(
			...
			if (TableProperties.resourceProvider == null) {
				...
				TableProperties.resourceProvider = new I18nResourceProvider() {

					// Always returns null
					@Override
					public String getResource(final String titleKey,
							final String property, final Tag tag,
	 ...
	 * @param key
	 *            property name
	 * @return boolean <code>true</code> if the property value is "true",
	...
	/**
	 * Returns an instance of a configured Class. Returns a configured Class
	 * instantiated callingClass.forName([configuration value]).
	 * 
	 * @param key
	 *            configuration key
	...
	private Object getClassPropertyInstance(final String key)
		...
		try {
			...
			throw new FactoryInstantiationException(this.getClass(), key,
					className, e);
		}
	}
	 ...
	 *            property name
	 * @param defaultValue
	 *            default value returned if property is not found or not a valid
	 *            int value
	...
	private int getIntProperty(final String key, final int defaultValue) {
		try {
			...
			TableProperties.log.warn(Messages.getString(
					"TableProperties.invalidvalue", //$NON-NLS-1$
					new Object[] { key, this.getProperty(key),
	...
	public String getPagingBannerAll() {
		return this
				.getProperty(TableProperties.PROPERTY_STRING_PAGING_BANNER_ALL);
	}
}
