...
/**
 * Created by IntelliJ IDEA. User: sg897500 Date: Mar 24, 2005 Time: 3:31:32 PM
 * Taken from: Helper class for generation of paging banners.
 * 
...
public class Pagination {
	 ...
	 */
	public Pagination(final Href baseHref, final String pageParameter) {
		this.href = baseHref;
	 ...
	 */
	public String getFormattedBanner(final String numberedPageFormat,
			final String numberedPageSelectedFormat,
			final String numberedPageSeparator, final String fullBanner) {
		final StringBuffer buffer = new StringBuffer(100);

		// numbered page list
		final Iterator pageIterator = this.pages.iterator();

		while (pageIterator.hasNext()) {

			// get NumberedPage from iterator
			final NumberedPage page = (NumberedPage) pageIterator.next();

			final Integer pageNumber = new Integer(page.getNumber());

			final String urlString = ((Href) this.href.clone()).addParameter(
					this.pageParam, pageNumber).toString();
			...
			if (page.getSelected()) {
				...
				buffer.append(MessageFormat.format(numberedPageFormat,
						pageObjects));
			}
