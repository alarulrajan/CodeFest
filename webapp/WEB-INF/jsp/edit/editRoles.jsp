...
.page_body {
	...
	<bean:define id="roles" name="edit/roles"
	...
	<form method="post" action="<html:rewrite page="/do/edit/roles"/>">
		<xplanner:isUserAuthorized name="project" permission="admin.edit.role">
			...
			<table class="objecttable" border="0" bgcolor="#f0f0f0">
				...
				<db:useBeans id="persons" type="net.sf.xplanner.domain.Person"
					where="hidden = false" order="name" />
				<logic:iterate id="personne" indexId="index" name="persons"
					type="net.sf.xplanner.domain.Person">
					<tr>
						<td><bean:write name="personne" property="name" /></td>
						<td><input type="hidden" name="personId[<%=index%>]"
							value='<%=personne.getId()%>'> <select
							name="personRole[<%=index%>]">
								<option value="viewer"
									<%= roles.isRoleSelected("viewer", personne.getId()) %>><bean:message
										key="person.editor.role.viewer" />
								</option>
								<option value="editor"
									<%= roles.isRoleSelected("editor", personne.getId()) %>><bean:message
										key="person.editor.role.editor" />
								</option>
								<option value="admin"
									<%= roles.isRoleSelected("admin", personne.getId()) %>><bean:message
