...
<xplanner:content definition="tiles:edit">
	...
	<form method="post" action="<html:rewrite page="/do/edit/person"/>"
		name="personEditor">
		<div id="editObject">
			<logic:present parameter="oid">
			...
			<table class="editor">
				...
				<tr>
					...
					<td><div class="borderedInput">
							<div>
								<html:text name="person" property="userIdentifier" />
							</div>
						</div></td>
				</tr>
				<tr>
					...
					<td><div class="borderedInput">
							<div>
								<html:text name="person" property="initials" />
							</div>
						</div></td>
				</tr>
				<tr>
					...
					<td><div class="borderedInput">
							<div>
								<html:text name="person" property="email" />
							</div>
						</div></td>
				</tr>
				<tr>
					<th nowrap valign="top"><bean:message
							key="person.editor.phone" /></th>
					<td><div class="borderedInput">
							<div>
								<html:text name="person" property="phone" />
							</div>
						</div></td>
				</tr>
				<xplanner:isUserAuthorized name="person" permission="hide">
					<tr>
						<th><b><bean:message key="person.editor.ishidden" />:</b></th>
						<td>
							<div class="borderedInput">
								<div>
									<html:select name="person" property="hidden">
            ...
            AuthenticatorImpl.getLoginModule(request).isCapableOfChangingPasswords()) {%>
				<tr>
					<th nowrap><logic:present parameter="oid">
					...
					<td><div class="borderedInput">
							<div>
								<html:password name="person" property="newPassword" />
							</div>
						</div></td>
				</tr>
				<tr>
					...
					<td><div class="borderedInput">
							<div>
								<html:password name="person" property="newPasswordConfirm" />
							</div>
						</div></td>
				</tr>
				<%}%>
				...
				<xplanner:isUserAuthorizedForAny name="projects"
					permissions="admin.edit.role">
					<tr>
						<td nowrap valign="top" colspan="2"><b><bean:message
									key="person.editor.roles" /> </b></td>
					</tr>
					<tr>

						<td><strong><bean:message
									key="person.roles.tableheading.project" /></strong></td>
						<td><strong><bean:message
									key="person.roles.tableheading.role" /></strong></td>
					</tr>
					<%int index = -1;%>
					<logic:iterate id="project" name="projects"
						type="net.sf.xplanner.domain.Project">
						<xplanner:isUserAuthorized name="project"
							permission="admin.edit.role">
							<tr>
								<td><bean:write name="project" property="name" /></td>
								<td>
									<% index++; %> <input type="hidden" name="projectId[<%=index%>]"
									value='<%=project.getId()%>'>
									<div class="borderedInput">
										<div>
											<select name="projectRole[<%=index%>]">
												<option value="none"
													<%= person.isRoleSelected("none", project.getId()) %>><bean:message
														key="person.editor.role.none" />
												</option>
												<option value="viewer"
													<%= person.isRoleSelected("viewer", project.getId()) %>><bean:message
														key="person.editor.role.viewer" />
												</option>
												<option value="editor"
													<%= person.isRoleSelected("editor", project.getId()) %>><bean:message
														key="person.editor.role.editor" />
												</option>
												<option value="admin"
													<%= person.isRoleSelected("admin", project.getId()) %>><bean:message
														key="person.editor.role.admin" />
												</option>
											</select>
										</div>
									</div>
				...
				<xplanner:isUserAuthorized resourceType="system"
					permission="sysadmin.promote">
					<tr>
						<td><b><bean:message key="person.editor.is_sysadmin" /></b></td>
						<td><input name="systemAdmin" type="checkbox" value="true"
							<%= person.isSysAdmin() ? "checked='checked'" : "" %>></td>
					</tr>
				</xplanner:isUserAuthorized>
				<tr>
