...
<%@ page
	import="com.technoetic.xplanner.forms.IterationStatusEditorForm"%><%@ page
	import="com.technoetic.xplanner.actions.UpdateTimeAction"%>
...
<xplanner:content>
	...
	<bean:define id="startedIterationsNr" name="startedIterationsNr"
	...
	<p>
		<span class="title"><h1>
				<bean:message key="iteration.status.editor.start_prefix" /></span> <a
			href="">${iteration.name}</a>
	</h1>
	</p>
	...
	<form method="post" action="<html:rewrite page="/do/start/iteration"/>"
		name="changeStatus">
		<bean:parameter id="returnto" name="returnto" />
		<input type="hidden" name="oid" value="${iteration.id}" /> <input
			type="hidden" name="fkey" value='<%= request.getParameter("fkey") %>' />
		<input type="hidden" name="returnto" value='<%=returnto%>' /> <input
			type="hidden" name="operation" value="start" /> <input type="hidden"
			name="iterationStartConfirmed" value="true" /> <input type="hidden"
			name="<%=IterationStatusEditorForm.SAVE_TIME_ATTR%>"
		...
		<div id="editObject">
			<table border="0">
				...
				<tr>
					<td><bean:message key="iteration.status.editor.message_2"
							arg0='<%=""+startedIterationsNr.intValue()%>' /> <br /> <input
						id="closeIterations" name="closeIterations" type="checkbox"
						value="on" checked="checked">
					<bean:message key="iteration.status.editor.message_3" /> <br /></td>
				</tr>
				<%}%>
				<tr>
					<td><br />
					<bean:message key="iteration.status.editor.message_4" /></td>
				</tr>
			...
			<table class="editor">
				<tr class="objecttable_buttons">
					<td>
						...
						value="<bean:message key="ok"/>" /> <input type="button"
						id="cancel" name="cancel" value="<bean:message key="cancel"/>"
