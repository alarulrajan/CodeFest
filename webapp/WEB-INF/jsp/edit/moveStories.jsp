...
<xplanner:content definition="tiles:edit">
	<bean:define id="moveStories" name="move/userstories"
	...
	<form name="moveStoriesForm" method="post"
		action="<html:rewrite page="/do/move/userstories"/>">
		<c:forEach var="storyId" items="<%= moveStories.getStoryIds() %>"
			varStatus="status">
			<input type="hidden" name="selected[${status.index}]"
				value="${storyId}" />
		</c:forEach>

		<input type="hidden" name="iterationId"
			value="<%= moveStories.getIterationId() %>" />
