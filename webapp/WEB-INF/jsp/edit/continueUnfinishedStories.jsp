...
<xplanner:content definition="tiles:edit">
	...
	<jsp:useBean id="iterationLoader"
		class="com.technoetic.xplanner.tags.IterationLoader" scope="request">
		<jsp:setProperty name="iterationLoader" property="pageContext"
			value='<%=pageContext%>' />
	</jsp:useBean>
	...
	<p class="highlighted_message">
		<bean:message key="iteration.status.editor.no_future_iteration" />
	</p>
	<% } else { %>

	<form id="continue" method="post"
		action="<html:rewrite page="/do/continue/unfinished/stories"/>">
		<div id="editObject">
			<input type="hidden" name="oid" value="${iteration.iterationId}">
			<input type="hidden" name="iterationId"
				value="${iteration.iterationId}"> <input type="hidden"
				name="merge" value="true"> <input type="hidden" name="fkey"
			...
			<table cellpadding="4" bgcolor="" class="editor">
				...
				<tr class="">
					<td colspan="2">
						<div class="borderedInput">
							<div>
								<html:select name="iteration" property="targetIterationId">
				...
				<tr class="objecttable_buttons">
					<td><input type="submit" name="action"
						id='<%= com.technoetic.xplanner.actions.ContinueUnfinishedStoriesAction.OK_ACTION %>'
						value="<bean:message key="ok"/>"></td>
					<td><input type="submit" name="action"
						id='<%= com.technoetic.xplanner.actions.ContinueUnfinishedStoriesAction.CANCEL_ACTION %>'
						value="<bean:message key="cancel"/>"></td>
				</tr>
			</table>
		</div>
	</form>
