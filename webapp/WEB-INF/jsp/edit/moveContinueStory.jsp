...
<xplanner:content definition="tiles:edit">
	<bean:define id="story" name="move/continue/userstory"
		type="com.technoetic.xplanner.forms.MoveContinueStoryForm" />
	...
	<form name="moveContinueForm" method="post"
		action="<html:rewrite page="/do/move/continue/userstory"/>">
		<div id="editObject">
			<input type="hidden" name="oid" value="${story.id}"> <input
				type="hidden" name="iterationId" value="${story.iterationId}">
			<input type="hidden" name="merge" value="true"> <input
				type="hidden" name="fkey"
				value='<%= request.getParameter("fkey") %>'> <input
				type="hidden" name="action"
				value='<%=com.technoetic.xplanner.actions.MoveContinueStoryAction.MOVE_ACTION%>' />
			...
			<table class="editor">
				...
				<tr>
					<td colspan="2">
						<div class="borderedInput">
							<div>
								<html:select name="story" property="targetIterationId">
				...
				<tr class="objecttable_buttons">
					<td><input type="submit" name="actionBtn"
						id='<%= com.technoetic.xplanner.actions.MoveContinueStoryAction.MOVE_ACTION %>'
						onclick="this.form.action.value='<%= com.technoetic.xplanner.actions.MoveContinueStoryAction.MOVE_ACTION %>'"
						value="<bean:message key="form.move"/>"></td>
					<td>
						<%if (!story.isFutureIteration()){%> <input type="submit"
						name="actionBtn"
						id='<%= com.technoetic.xplanner.actions.MoveContinueStoryAction.CONTINUE_ACTION %>'
