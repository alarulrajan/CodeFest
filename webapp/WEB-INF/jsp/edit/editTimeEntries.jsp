...
<xplanner:content definition="tiles:edit">
	...
	<bean:define id="entries" name="edit/time"
...
// -->
	...
	<body>
		...
		<form method="post" name="timelog"
			action="<html:rewrite page="/do/edit/time"/>">
			<div id="objecttable" class="objecttable time">
				...
				<input type="hidden" name="fkey" value='<%=fkey%>'> <input
					type="hidden" name="taskId" value='<%=fkey%>'>
				<table>
					<thead>
						<tr>
							...
							<th class="small_th" nowrap><bean:message
									key="timelog.editor.duration" /></th>
							<th nowrap><span id="remainingHoursLabel"
								style="color: black"> <bean:message
										key="timelog.editor.remainingHours" /></span></th>
							...
							<th>
								...
								<div class="plus" style="cursor: pointer; float: left;">
									<html:img page="/images/plus.png" alt="delete" border="0" />
								</div>
								<p class="person2" style="cursor: pointer; display: none;">
									<bean:message key="timelog.editor.person2" />
								</p>

							</th>
							<th><bean:message key="timelog.editor.description" /></th>
							<th><html:img page="/images/delete1.png" alt="delete"
									border="0" /></th>
						</tr>
					</thead>
					<%
   ...
   %>
					<tr>
						<td class="objecttable small_th_1"
							<%= entries.isIntervalReadOnly(row) ? "bgcolor='#f0f0f0'" : "" %>>
							<% if (entries.isIntervalReadOnly(row)) { %> &nbsp; <% } else { %> <input
							type="text" size="12" name="startTime[<%=row%>]"
							value='<%= entries.getStartTime(row) %>'
							onchange='entryChanged()' /> <% } %>
						</td>
						<td class="objecttable small_th_2"
							<%= entries.isIntervalReadOnly(row) ? "bgcolor='#f0f0f0'" : "" %>>
							<% if (entries.isIntervalReadOnly(row)) { %> &nbsp; <% } else { %> <input
							type="text" size="16" name="endTime[<%=row%>]"
							value='<%= entries.getEndTime(row) %>' onchange='entryChanged()' />
							<% } %>
						</td>
						<td class="objecttable small_th_3"><input type="text"
							size="16" name="reportDate[<%=row%>]"
							value='<%= entries.getReportDate(row) %>'></td>
						<td class="objecttable small_th_4" align="right">
							<% if (entries.isDurationReadOnly(row)) { %> <% if (!entries.isEmpty(row)) { %>
							<%= entries.getDuration(row) %> <input type="hidden"
							name="duration[<%= row %>]"
							value='<%= entries.getDuration(row) %>' onchange='entryChanged()' />
							<% } else { %> &nbsp; <% } %> <% } else { %> <input type="text"
							size="2" name="duration[<%=row%>]" onchange='entryChanged()'
							value='<%= entries.getDuration(row) == null ? "" : entries.getDuration(row).toString() %>'>
							<% } %>
						</td>
						<td class="objecttable small_th_5" align="right">
							<% if (row == rowcount - 1) { %> <input type="text" size="2"
							name="remainingHours" onChange="changeRemainingHours()"
							value='<%= task.getRemainingHours() %>' /> <% } else { %> &nbsp; <% } %>
						</td>
						<td class="objecttable small_th_6"><html:select
								name="entries" property='<%="person1Id["+row+"]"%>'
						...
						<td class="objecttable small_th_7">
							<div class="hiden" style="display: none;">
								<html:select name="entries" property='<%="person2Id["+row+"]"%>'
    ...
    $("div.hiden").show("fast");
						...
						<td class="objecttable small_th_8"><html:text name="entries"
								property='<%="description["+row+"]"%>' size="50"
								maxlength="<%=String.valueOf(TimeEditorForm.MAX_DESCRIPTION_LENGTH)%>" />
							<!--input type="text" size="50" name="description[<%//=row%>]" value='<%//=entries.getDescription(row)%>'-->
						</td>
						<td align="center" class="objecttable small_th_9"><input
							type="hidden" name="entryId[<%=row%>]"
							value='<%=entries.getEntryId(row)%>'> <% if (!entries.isEmpty(row)) { %>
							<input type="checkbox" name="deleted[<%=row%>]" value="true"
							onChange="entryChanged()" /> <% } else { %> <input type="hidden"
							name="emptyRow[<%=row%>]" value="true" /> <% } %></td>
					...
					<% } /* for loop */ %>
					<tr>
						<td class="objecttable_buttons" nowrap colspan="9"><input
							type="hidden" name="action"
							value='<%= UpdateTimeAction.UPDATE_TIME_ACTION %>' /> <input
							type="submit" name="submit"
							value="<bean:message key="form.update"/>">&nbsp; <input
							type="button" name="reset"
							value="<bean:message key="form.reset"/>"
							onclick="javascript: window.location.reload()">&nbsp; <input
							type="button" name="action"
							value="<bean:message key="timelog.editor.insert_time"/>"
							onClick='setTime(<%=rowcount%>)'> &nbsp;<bean:message
								key="timelog.editor.timeformat" /></td>
					</tr>
				</table>
