...
<xplanner:content definition="tiles:edit">
	...
	<bean:define id="importPeople" name="import/people"
	...
	<span class="help_message"> <bean:message
			key="people.import.editor.importFileFormat" /> <br /> <bean:message
			key="import.editor.download.template" /> <html:link
			page='<%= "/" + messages.getMessage("people.import.download.template.file.name")%>'>
			<bean:message key="import.editor.download.template.location" />
		</html:link> <br />
	</span>

	<%-- rewrite is necessary for cookie-less sessions --%>
	<form method="post" action="<html:rewrite page="/do/import/people"/>"
		enctype="multipart/form-data" name="importPeople">
		<div id="editObject">
			...
			<table class="editor">
				<tr>
					...
					<td><html:file name="importPeople" property="formFile"
							value='<%= request.getParameter("formFileName")%>' /></td>
				</tr>
				<tr>
					<td nowrap colspan="2" class="objecttable_buttons"><input
	...
	<logic:notEmpty name="importPeople" property="results">
		<p>
			<b><bean:message key="people.import.editor.results" /></b>
		</p>
