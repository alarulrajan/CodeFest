...
<%@ page import="org.apache.struts.*"%><%@ page
...
<xplanner:content definition="tiles:edit">
	<bean:define id="task" name="edit/task"
	...
	<form action="<html:rewrite page="/do/edit/task"/>" method="POST"
		name="taskEditor">
		<div id="editObject">
			<logic:present parameter="oid">
			...
			<input type="hidden" name="fkey" value='<%=fkey%>'> <input
				type="hidden" name="userStoryId" value='<%=fkey%>'> <input
			...
			<table class="editor">
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="task.editor.name" /> </b></th>
					<td class="secondColumn"><div class="borderedInput">
							<div>
								<html:text name="task" property="name" size="40" />
							</div>
						</div></td>
				</tr>
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="task.editor.type" /> </b></th>
					<td><bean:define id="messages"
							name='<%= Globals.MESSAGES_KEY %>' scope="application"
							type="org.apache.struts.util.MessageResources" /> <!-- DEBT(DYNAMICFIELDS) Make task type dynamic and create a html:option derived tag -->
						<div class="borderedInput">
							<div>
								<html:select name="task" property="type">
									<html:option
										value='<%= messages.getMessage("task.type.feature") %>'>
										<bean:message key="task.type.feature" />
									</html:option>
									<html:option
										value='<%= messages.getMessage("task.type.debt") %>'>
										<bean:message key="task.type.debt" />
									</html:option>
									<html:option
										value='<%= messages.getMessage("task.type.defect") %>'>
										<bean:message key="task.type.defect" />
									</html:option>
									<html:option
										value='<%= messages.getMessage("task.type.ftest") %>'>
										<bean:message key="task.type.ftest" />
									</html:option>
									<html:option
										value='<%= messages.getMessage("task.type.atest") %>'>
										<bean:message key="task.type.atest" />
									</html:option>
									<html:option
										value='<%= messages.getMessage("task.type.overhead") %>'>
				...
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="task.editor.disposition" /> </b></th>
					<td>
						<div class="borderedInput">
							<div>
								<html:select name="task" property="dispositionName">
									<html:option value='<%= TaskDisposition.PLANNED.getName() %>'>
										<bean:message
											key='<%= TaskDisposition.PLANNED.getNameKey() %>' />
									</html:option>
									<html:option
										value='<%= TaskDisposition.DISCOVERED.getName() %>'>
										<bean:message
											key='<%= TaskDisposition.DISCOVERED.getNameKey() %>' />
									</html:option>
									<html:option value='<%= TaskDisposition.ADDED.getName() %>'>
										<bean:message key='<%= TaskDisposition.ADDED.getNameKey() %>' />
									</html:option>
									<html:option
										value='<%= TaskDisposition.CARRIED_OVER.getName() %>'>
										<bean:message
											key='<%= TaskDisposition.CARRIED_OVER.getNameKey() %>' />
									</html:option>
								</html:select>
							</div>
						</div>
				...
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="task.editor.estimated_hours" /></b></th>
					<td><div class="borderedInput">
							<div>
								<html:text name="task" property="estimatedHours" size="6" />
							</div>
						</div></td>
				</tr>
				<tr>
					<td colspan="2" valign="top">
						<table width="100%" border="0">
							<tr>
								<td><b><bean:message key="task.editor.description" /></b> (<bean:message
										key="form.maxtext" />)</td>
				...
				<tr>
					<td colspan="2"><div class="borderedInput">
							<div>
								<html:textarea name="task" property="description" cols="60"
									rows="10" />
							</div>
						</div></td>
				</tr>
