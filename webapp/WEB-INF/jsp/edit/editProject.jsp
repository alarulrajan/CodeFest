...
<xplanner:content definition="tiles:edit">
	<bean:define id="project" name="edit/project"
   ...
   function deleteNotifiedPerson(userId){
	...
	<form name="projectEditorForm" method="post"
		action="<html:rewrite page="/do/edit/project"/>">
		<div id="editObject" class="editProject">
			<input type="hidden" name="action" value="" /> <input type="hidden"
				name="personToDelete" value="" />
			<logic:present parameter="oid">
			...
			<table class="editor">
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="project.editor.name" />:</b></th>
					<td class="secondColumn">
						<div class="borderedInput">
							<div>
								<html:text name="project" property="name" size="40" />
							</div>
						</div>
					</td>
				</tr>
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="project.editor.description" />: </b></th>
					<td align="right"><%@ include
							file="/WEB-INF/jsp/common/formattingHelpLink.jsp"%>
					</td>
				</tr>
				<tr>
					<td colspan="2">
						<div class="borderedInput">
							<div>
								<html:textarea name="project" property="description" cols="60"
									rows="10" />
							</div>
						</div>
					</td>
				...
				<tr>
					<td colspan="2"><b><bean:message
								key="project.editor.wikiurl" /></b> <%-- String wikiURL = new com.technoetic.xplanner.XPlannerProperties().getProperty("twiki.scheme.wiki", project, "http://");
                  request.setAttribute("twiki.scheme.wiki", wikiURL); %>
                <input name="twiki.scheme.wiki" type="text" size="50" value='<%=wikiURL%>'>  --%>
						<div class="borderedInput">
							<div>
								<html:text name="project" property="wikiUrl" size="50" />
							</div>
						</div></td>
				</tr>

				<tr>
					<td nowrap colspan="2" class="objecttable_buttons"><logic:present
							parameter="oid">
							<input type="submit" value="<bean:message key="form.update"/>"
								onclick="this.form.action.value='<%= EditObjectAction.UPDATE_ACTION%>'" />
						</logic:present> <logic:notPresent parameter="oid">
							<input type="submit" value="<bean:message key="form.create"/>"
								onclick="this.form.action.value='<%= EditObjectAction.CREATE_ACTION%>'">
						</logic:notPresent> <input type="reset" value="<bean:message key="form.reset"/>">
					</td>
				</tr>
			</table>
			...
			<logic:present parameter="oid">
				<p>
					<span class="title"><bean:message
							key="time_entry_notification.editor.editor_prefix" /></span>
				</p>
				<p>
					<bean:message key="time_entry_notification.editor.explanation" />
				</p>
				<table id="remarks" class="editor">
					<tr>
						<th nowrap><bean:message
								key="time_entry_notification.editor.userToAdd" /></th>
						<td class="addRecipient">
							<div class="borderedInput">
								<div>
									<html:select property="personToAddId" name="project">
										<html:option value="0">
											<bean:message key="person.not_assigned" />
										</html:option>
										<xplanner:personOptions />
									</html:select>
								</div>
							</div>
							<div class="objecttable_buttons">
								<input type="submit" name="send" id="add"
									value="<bean:message key="form.add"/>"
									onclick="this.form.action.value = '<%=UpdateTimeNotificationReceivers.ADD%>'">
							</div>
						</td>
					</tr>
					<tr>
						<td colspan="2"><logic:notEmpty name="project"
								property="people">
								<bean:message
									key="time_entry_notification.editor.receivers_list" />
							</logic:notEmpty>
							<table>
								<logic:notEmpty name="project" property="people">
									<thead>
										<tr>
											...
											<th nowrap><bean:message
													key="time_entry_notification.editor.user_name" /></th>
										</tr>
									</thead>
									<tbody>
										<logic:iterate id="personInfo" name="project"
											property="people">
											<tr>
												<td align="center"><a href="#"
													onclick="return deleteNotifiedPerson('<%=((ProjectEditorForm.PersonInfo)personInfo).getId()%>')"><html:img
															page="/images/delete.png" border="0" /></a></td>
												<td><bean:write name="personInfo" property="name" /></td>
											</tr>
										</logic:iterate>
									</tbody>
								</logic:notEmpty>
