...
<xplanner:content definition="tiles:edit">
	<bean:define id="story" name="edit/userstory"
	...
	<bean:define id="messages" name='<%= Globals.MESSAGES_KEY %>'
		scope="application" type="org.apache.struts.util.MessageResources" />
	<%-- rewrite is necessary for cookie-less sessions --%>
	<form method="post" action="<html:rewrite page="/do/edit/userstory"/>"
		name="storyEditor">
		<div id="editObject">
			<logic:present parameter="oid">
			...
			<input type="hidden" name="iterationId" value='<%=fkey%>' /> <input
				type="hidden" name="id" value='<%= story.getId() %>'> <input
				type="hidden" name="returnto" value='<%=returnto%>'> <input
				type="hidden" name="fkey" value='<%=fkey%>'>

			<table class="editor">
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="story.editor.name" /> </b></th>
					<td class="secondColumn"><div class="borderedInput">
							<div>
								<html:text name="story" property="name" size="40" />
							</div>
						</div></td>
				</tr>
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="story.editor.disposition" /> </b></th>
					<td><div class="borderedInput">
							<div>
								<html:select name="story" property="dispositionName">
									<html:option value='<%= StoryDisposition.PLANNED.getName()%>'>
										<bean:message
											key='<%= StoryDisposition.PLANNED.getNameKey() %>' />
									</html:option>
									<html:option
										value='<%= StoryDisposition.CARRIED_OVER.getName()  %>'>
										<bean:message
											key='<%= StoryDisposition.CARRIED_OVER.getNameKey() %>' />
									</html:option>
				...
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="story.editor.customer" /> </b></th>
					<td><div class="borderedInput">
							<div>
							...
							<div>
								<html:select name="story" property="statusName">
									...
									<html:option value='<%= StoryStatus.IMPLEMENTED.getName() %>'>
										<bean:message
											key='<%= StoryStatus.IMPLEMENTED.getNameKey() %>' />
									</html:option>
				...
				<tr>
					<th valign="top"><b><bean:message
								key="story.editor.priority" /></b></th>
					<td>
						<div class="borderedInput">
							<div>
								<html:text name="story" property="priority" size="40" />
							</div>
						</div>
					</td>
				<tr>
				<tr>
					...
					<td>
						<div class="borderedInput">
							<div>
								<html:text name="story" property="orderNo" size="40" />
							</div>
						</div>
					</td>
				<tr>
					<th nowrap valign="top"><b><bean:message
								key="story.editor.estimated_hours" /> </b></th>
					<td><logic:greaterThan name="story"
							property="taskBasedEstimatedHours" value="0">
							<%= DecimalFormat.format(pageContext, story.getTaskBasedEstimatedHours()) %>
						</logic:greaterThan> <logic:equal name="story" property="taskBasedEstimatedHours"
							value="0">
							<div class="borderedInput">
								<div>
									<html:text name="story"
										property='<%=UserStory.ESTIMATED_HOURS%>' size="5" />
								</div>
							</div>
						</logic:equal></td>
				</tr>
				...
				<tr>
					<td colspan="2"><div class="borderedInput">
							<div>
								<html:textarea name="story" property="description" cols="60"
									rows="10" />
							</div>
						</div></td>
				</tr>
