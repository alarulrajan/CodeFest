...
<go
	href="/xplanner/do/mobile/view/story?oid=<%=task.getUserStory().getId()%>" />
</do>
<do type="accept" label="Iteration">
<go
	href="/xplanner/do/mobile/view/iteration?oid=<%=story.getIteration().getId()%>" />
</do>
...
<p>
	...
	<br /> <b>Status:</b>
	<%= task.isCompleted() ? "Complete" : "Open" %><br /> <b>Act./Est.:</b>
	<%= formatter.format(task.getActualHours()) %>/<%= formatter.format(task.getEstimatedHours()) %><br />
	<logic:greaterThan name="task" property="acceptorId" value="0">
		...
		<b>Acceptor:</b>
		<a href="/xplanner/do/mobile/view/person?oid=<%= acceptor.getId() %>"><bean:write
				name="acceptor" property="initials" /></a>
		<br />
	</logic:greaterThan>
