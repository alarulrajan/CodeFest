...
<xplanner:content definition="tiles:edit">
	...
	<bean:define id="importForm" name="import"
		type="com.technoetic.xplanner.forms.ImportForm" />
	...
	<span class="help_message"> <bean:message
			key="import.editor.helpMessage" /><br />
	<br /> <bean:message key="import.editor.download.template" /> <html:link
			page='<%= "/" + messages.getMessage("import.editor.download.template.file.name")%>'>
			<bean:message key="import.editor.download.template.location" />
		</html:link> <br />
	</span>

	<form method="post" action="<html:rewrite page="/do/import/stories"/>"
		...
		<div id="editObject">
			...
			<table class="editor">
				<tr>
					<th class="required"><bean:message
							key="form.required.field.indicator" /></th>
				...
				<tr>
					<th class="required"><bean:message
							key="form.required.field.indicator" /></th>
					<th nowrap><b><bean:message
								key="import.editor.titleColumn" /></th>
				...
				<tr>
					<th class="required"><bean:message
							key="form.required.field.indicator" /></th>
					<th colspan="2"><span class="required"><bean:message
								key="form.required.field" /></span></th>
				</tr>
				<tr>
					<td nowrap colspan="3" class="objecttable_buttons"><input
						type="hidden" name="action"
						value='<%= com.technoetic.xplanner.actions.EditObjectAction.UPDATE_ACTION %>'>
						<input type="submit"
						value="<bean:message key="import.button.label"/>"> <input
						type="reset" name="reset" value="<bean:message key="form.reset"/>">
					</td>
				</tr>
			</table>
		</div>
	...
	<p class="highlighted_message">
		<bean:message key="stories.import.imported_stories"
			arg0='<%=""+importForm.getResults().size()%>' />
	</p>
	<% } %>
</xplanner:content>