...
<%@ page
	...
	import="com.technoetic.xplanner.util.LocaleUtil"%><%@ page
	import="com.technoetic.xplanner.actions.SystemInfoAction"%>
...
.warning {
	...
	text-align: center;
		...
		<tr>
			<td valign="top"><span class="title"><bean:message
						key='<%=titleKey%>' /></span>
				<p>
					<%
  if (!isSystemInfo) {
  ...
  if (exception != null && exception.getMessage() != null) {
    ...
    %>
					<span class="error_message"> <%= message %> <%} else {%> <bean:message
							key="error.unknownError" /> <%}%> <br />
					<bean:message key="error.moreInfo" />
					</span>
				</p> <%
       ...
       %>
				...
				<br /> <%
          if (exception != null) {
        %> <xplanner:box title="Cause">
					<pre><%= exception.toString() %></pre>
				</xplanner:box> <xplanner:box title="Stack Trace">
					<pre>
        ...
        %>
				<div class="sectionTitle">System Environment</div> <xplanner:boxedList
					title="Build" keyValues="<%=systemInfo.getBuildInfo()%>" /> <xplanner:boxedList
					title="Database" keyValues="<%=systemInfo.getDatabaseInfo()%>" /> <xplanner:boxedList
					title="App Server" keyValues="<%=systemInfo.getAppServerInfo()%>" />
				<xplanner:boxedList title="Memory"
					keyValues="<%=systemInfo.getJVMStatistics()%>" /> <xplanner:boxedList
					title="System" keyValues="<%=systemInfo.getSystemProperties()%>" />
        ...
        %> <xplanner:boxedList title="General">
				...
				</xplanner:boxedList> <xplanner:boxedList title="Parameters"
					keyValues="<%=new TreeMap(request.getParameterMap())%>" /> <xplanner:boxedList
					title="Attributes"
					keyValues="<%=BoxedListTag.getRequestAttributeMap(request)%>" /> <%
          }
          catch (Throwable t) {
