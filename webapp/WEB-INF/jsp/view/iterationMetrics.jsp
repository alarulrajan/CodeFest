...
<%@ page
	import="org.apache.struts.Globals,com.technoetic.xplanner.format.DecimalFormat,
...
<xplanner:content definition="tiles:view">
	...
	<jsp:useBean id="metrics"
		class="com.technoetic.xplanner.metrics.IterationMetrics"
		scope="request">
	</jsp:useBean>

	<div class="title">
		<h1>
			<bean:message key="iteration.metrics.total_hours" />
			<%= DecimalFormat.format(pageContext, metrics.getTotalHours()) %>
		</h1>
	</div>

	<div class="title">
		<h1>
			<bean:message key="iteration.metrics.avr_pair" />
			<%= DecimalFormat.format(pageContext, metrics.getTotalPairedPercentage()) %>%
		</h1>
	</div>
	...
	<p class="highlighted_message">
		<dt:table id="totalTable" htmlId="totalTable" uid="totalTable"
			name="metrics" defaultsort="1" property="developerTotalTime"
	...
	<dt:column sortable="true" sortProperty="name"
		title='<%= messages.getMessage("iteration.metrics.tableheading.developer") %>'
		...
		<xplanner:link page="/do/view/person" paramId="oid"
			paramName="developerMetrics" paramProperty="id">
	        ${developerMetrics.name}</xplanner:link>
		<% } else { %>
            ${developerMetrics.name}
         <% } %>
	</dt:column>
	<dt:column align="right" nowrap="true" sortable="true"
		sortProperty="hours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.total") %>'
	...
	<dt:column align="right" nowrap="true" sortable="true"
		sortProperty="hours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.paired_hours") %>'
	...
	<dt:column align="right" nowrap="true" sortable="true"
		sortProperty="hours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.solo_hours") %>'
	...
	<dt:column nowrap="true" sortable="true" sortProperty="hours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.paired_outof_total") %>'
	...
	<div class="title">
		<h1>
			<bean:message key="iteration.metrics.accepted_hours" />
		</h1>
	</div>
	<p class="highlighted_message">
		<dt:table id="acceptedTable" htmlId="acceptedTable"
			uid="acceptedTable" defaultsort="1" name="metrics"
			property="developerAcceptedTime" styleClass="objecttable"
			requestURI="">
	</p>
	...
	<dt:column sortable="true"
		...
		sortProperty="name"
		title='<%= messages.getMessage("iteration.metrics.tableheading.developer") %>'>
		<logic:notEqual name="developerMetrics" property="acceptedHours"
			value="0">
			<% if (!developerMetrics.getName().equals(IterationMetrics.UNASSIGNED_NAME) ) { %>
			<xplanner:link page="/do/view/person" paramId="oid"
				paramName="developerMetrics" paramProperty="id">
	...
	<dt:column align="right" sortable="true" sortProperty="acceptedHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.total") %>'
	...
	<dt:column align="right" sortable="true" sortProperty="acceptedHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.story") %>'
	...
	<dt:column align="right" sortable="true" sortProperty="acceptedHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.task") %>'
	...
	<dt:column align="right" sortable="true" sortProperty="ownTaskHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.worked_hours") %>'
	...
	<dt:column align="right" sortable="true"
		sortProperty="remainingTaskHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.remaining_hours") %>'
		htmlTitle='<%= messages.getMessage("iteration.metrics.headertooltip.remaining_hours")%>'>
		<logic:notEqual name="developerMetrics" property="remainingTaskHours"
			value="0">
			<%= DecimalFormat.format(pageContext, developerMetrics.getRemainingTaskHours(),"&nbsp;") %>
		</logic:notEqual>
	</dt:column>
	<dt:column sortable="true" sortProperty="ownTaskHours"
		title='<%= messages.getMessage("iteration.metrics.tableheading.worked_outof_accepted") %>'
		...
		<logic:notEqual name="developerMetrics" property="acceptedHours"
			...
			<table id="metrics1">
             ...
             %>
				...
				<td class="pairedHours" width='<%= ownTaskWorkedHoursWidth %>'
					bgcolor="#A0A0A0"></td>
				<% } %>
				<% if (acceptedHoursWithNoWorkedWidth > 0) { %>
				<td class="unpairedHours" align="right"
					width='<%= acceptedHoursWithNoWorkedWidth %>' bgcolor="#C0C0C0"></td>
				<% } %>
			</table>
		</logic:notEqual>
	</dt:column>
