...
<%@ page
	import="com.technoetic.xplanner.actions.AbstractAction,
...
<xplanner:content definition="tiles:view">
	...
	<tiles:put name="progress" direct="true">
		<div class="iterationCard">
	        ...
	        ${acceptor.name}</xplanner:link></td>
					...
					<td><%= DecimalFormat.format(pageContext, task.getEstimatedHours()) %>
						<logic:greaterThan name="task" property="estimatedOriginalHours"
							value="0">
          (<%= DecimalFormat.format(pageContext, task.getEstimatedOriginalHours()) %>)
				...
				<tr>
					...
					<td nowrap><xplanner:formatDate formatKey="format.date"
							name="task" property="createdDate" /></td>
	...
	<xplanner:isUserAuthorized name="task" permission="edit">
		...
		<logic:equal name="task" property="completed" value="true">
			<form class="completeBtn" method="post" name="completion"
				action="<html:rewrite page="/do/edit/task"/>">
				<input type="hidden" name="oid" value='<%=task.getId()%>' /> <input
					type="hidden" name="action" value="Update" /> <input type="hidden"
					name="merge" value="true" /> <input type="hidden" name="returnto"
					value="/do/view/task?oid=<%=task.getId()%>" /> <input type="hidden"
					name="completed" value="false" /> <input type="submit"
					name="reopen" value="<bean:message key="task.label.reopen"/>" />
			</form>
		</logic:equal>
		...
		<logic:equal name="task" property="completed" value="false">
			<form class="completeBtn" method="post" name="completion"
				...
				<input type="hidden" name="oid" value='<%=task.getId()%>' /> <input
					type="hidden" name="action" value="Update" /> <input type="hidden"
					name="merge" value="true" /> <input type="hidden" name="returnto"
					value="/do/view/task?oid=${task.id}" /> <input type="hidden"
					name="completed" value="true" /> <input type="submit"
					name="complete" value="<bean:message key="task.label.complete"/>" />
			</form>
		</logic:equal>
...
and calendar-function cannot process the "/"-character -->
	...
	<logic:greaterThan name="entryCount" value="0">
		<p class="table_label">
			<bean:message key="task.label.timelog" />
		</p>
				...
				<logic:iterate name="entries" scope="page" id="entry" indexId="n"
					...
					<tr>
						<td nowrap align="center"
							<%= entry.getStartTime() == null ? "bgcolor='#f0f0f0'" : "" %>>
							<xplanner:formatDate formatKey="format.datetime" name="entry"
								property="startTime" />
						</td>
						<td nowrap
							<%= entry.getEndTime() == null ? "bgcolor='#f0f0f0'" : "" %>>
							<xplanner:formatDate formatKey="format.datetime" name="entry"
								property="endTime" />
						</td>
						<td nowrap align="center"
							<%= entry.getReportDate() == null ? "bgcolor='#f0f0f0'" : "" %>>
							<xplanner:formatDate formatKey="format.date" name="entry"
								property="reportDate" />
						</td>
						<td nowrap
							<%= entry.getDuration() == 0 ? "bgcolor='#f0f0f0'" : "" %>>
							<logic:greaterThan name="entry" property="duration" value="0">
								<%= DecimalFormat.format(pageContext, entry.getDuration()) %>
							</logic:greaterThan>
						</td>
						<td nowrap><logic:notEqual name="entry" property="person1Id"
								value="0">
								<db:useBean id="person1" type="net.sf.xplanner.domain.Person"
									oid='<%= new Integer(entry.getPerson1Id()) %>' />
								<xplanner:link page="/do/view/person" paramId="oid"
									paramName="person1" paramProperty="id">
                    ${person1.name}</xplanner:link>
							</logic:notEqual> <logic:notEqual name="entry" property="person2Id" value="0">
								<db:useBean id="person2" type="net.sf.xplanner.domain.Person"
									oid='<%= new Integer(entry.getPerson2Id()) %>' />
								<xplanner:link page="/do/view/person" paramId="oid"
									paramName="person2" paramProperty="id">
                    ${person2.name}</xplanner:link>
							</logic:notEqual></td>
						<td align="left"
							<logic:empty name="entry" property="description">"bgcolor='#f0f0f0'"</logic:empty>>
	...
	<tiles:put name="actions" direct="true">
		<xplanner:isUserAuthorized name="task" permission="edit">
			<xplanner:actionButtons name="task" id="action">
	...
	<tiles:put name="globalActions" direct="true">
		...
		<xplanner:link page="/do/view/history">
			...
			<xplanner:linkParam id='<%= AbstractAction.TYPE_KEY %>'
				value='<%= Hibernate.getClass(task).getName() %>' />
		</xplanner:link>
	</tiles:put>
