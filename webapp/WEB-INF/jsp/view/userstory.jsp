...
<%@ page
	import="com.technoetic.xplanner.format.*,
...
<xplanner:content definition="tiles:view">
	...
	<tiles:put name="progress" direct="true">
		<div class="iterationCard">
			<div class="iterationCardHeader">
				<strong><bean:message key="story.prefix" /> <span
					class="object_name"><%=story.getName()%> [id=<%=story.getId()%>]</span>
				</strong>
				<xplanner:progressBar actual='<%= story.getCachedActualHours() %>'
					estimate='<%= visualEstimation %>'
					complete='<%= story.isCompleted() %>' width="150" height="16" />
			</div>
			...
			<table class="info">
				<tr>
					<th>Priority:</th>
					<td><bean:write name="story" property="priority" /></td>
					<th nowrap><bean:message key="story.label.estimated_hours" />
					</th>
				...
				<tr>
					<logic:notEmpty name="story" property="customer">
						<th><bean:message key="story.label.customer" /></th>
						<td><xplanner:link page="/do/view/person" paramId="oid"
								paramName="story" paramProperty="customer.id">
								<bean:write name="story" property="customer.name" />
							</xplanner:link></td>
					</logic:notEmpty>
				...
				<logic:notEqual name="story" property="trackerId" value="0">
					<tr>
						<th><bean:message key="story.label.tracker" /></th>
						<td><db:useBean id="tracker"
								type="net.sf.xplanner.domain.Person"
								oid='<%= new Integer(story.getTrackerId()) %>' /> <xplanner:link
								page="/do/view/person" paramId="oid" paramName="tracker"
								paramProperty="id">
         ${tracker.name}</xplanner:link></td>
						<th nowrap><bean:message key="story.label.remaining_hours" />
						</th>
	...
	<logic:greaterThan name="taskCount" value="0">
		<xplanner:writableTable wholeCollection='<%=story.getTasks() %>'
			permissions="edit,delete" id="objecttable" defaultsort="3"
			defaultorder="ascending" name="requestScope.story" property="tasks"
			styleClass="objecttable" cellspacing="0" requestURI=""
			decorator="com.technoetic.xplanner.tags.displaytag.TaskDecorator">
			<bean:define id="task" name="objecttable"
				type="net.sf.xplanner.domain.Task" />
			<xplanner:actionButtonsColumn name="task" id="action"
				title='<%= messages.getMessage("tasks.tableheading.actions") %>'>
				<xplanner:isUserAuthorized name="task"
			...
			<dt:column
				...
				<html:link page="/do/view/task" paramId="oid" paramName="task"
					paramProperty="id">
	        ${task.id}</html:link>
			</dt:column>
			<dt:column sortable="true" sortProperty="name"
				title='<%= messages.getMessage("tasks.tableheading.name")%>'>
				<html:link styleClass="name" page="/do/view/task" paramId="oid"
					paramName="task" paramProperty="id">
	        ${task.name}</html:link>
			</dt:column>
			<dt:column align="center" sortable="true"
				title='<%= messages.getMessage("tasks.tableheading.type")%>'>
        ${task.type}
    </dt:column>
			<dt:column sortable="true" sortProperty="percentCompleted"
				title='<%= messages.getMessage("tasks.tableheading.progress")%>'>
				<xplanner:progressBar actual='<%= task.getActualHours() %>'
					estimate='<%= task.getActualHours() + task.getRemainingHours() %>'
					complete='<%= task.isCompleted()%>' height="13" width="100%" />
			</dt:column>
			<dt:column sortable="true" sortProperty="acceptorName"
				title='<%= messages.getMessage("tasks.tableheading.acceptor")%>'>
				<logic:notEqual name="task" property="acceptorId" value="0">
					...
					<xplanner:link page="/do/view/person" paramId="oid"
						paramName="acceptor" paramProperty="id">
		        ${acceptor.initials}</xplanner:link>
				</logic:notEqual>
				<logic:equal name="task" property="acceptorId" value="0">&nbsp;</logic:equal>
			</dt:column>
			<dt:column sortable="true" sortProperty="estimatedOriginalHours"
				title='<%= messages.getMessage("tasks.tableheading.estimate.original") %>'
				align="right">
				<%= DecimalFormat.format(pageContext, task.getEstimatedOriginalHours()) %>
			</dt:column>
			<dt:column align="right" sortProperty="estimatedHours"
				sortable="true"
				title='<%= messages.getMessage("tasks.tableheading.estimated_hours")%>'>
      ...
      if (!"image".equals(new XPlannerProperties().getProperty("xplanner.progressbar.impl"))) { %>
			<dt:column align="right" sortProperty="actualHours" sortable="true"
				title='<%= messages.getMessage("tasks.tableheading.actual_hours")%>'>
				<%= DecimalFormat.format(pageContext, task.getActualHours()) %>
			</dt:column>
			<%}%>
			<dt:column align="right" sortProperty="remainingHours"
				sortable="true"
				title='<%= messages.getMessage("tasks.tableheading.remaining_hours")%>'>
				<%= DecimalFormat.format(pageContext, task.getRemainingHours()) %>
			</dt:column>
			<dt:column align="center" sortable="true"
				title='<%= messages.getMessage("tasks.tableheading.disposition")%>'>
	...
	<logic:equal name="taskCount" value="0">
		<p class="highlighted_message">
			<bean:message key="story.no_tasks" />
		</p>
	</logic:equal>
	...
	<tiles:put name="globalActions" direct="true">
		...
		<xplanner:link page="/do/view/history">
			...
			<xplanner:linkParam id='<%= AbstractAction.TYPE_KEY %>'
				value='<%= Hibernate.getClass(story).getName() %>' />
		</xplanner:link>
	</tiles:put>
