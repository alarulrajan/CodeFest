...
<xplanner:content definition="tiles:view">
	...
	<c:if test="${fn:length(iterations) gt 0 }">
		<xplanner:writableTable wholeCollection='${iterations}'
			permissions="edit,delete"
			rowDecorator='<%= new IterationRowDecorator() %>' id="objecttable"
			name="pageScope.iterations" styleClass="objecttable" requestURI=""
			defaultorder="descending" defaultsort="5" pagesize="10">
			<dt:setProperty name="paging.banner.placement" value="bottom" />
			<bean:define id="iteration" name="objecttable"
				type="net.sf.xplanner.domain.Iteration" />
			<xplanner:actionButtonsColumn styleClass="image" name="iteration"
				id="action"
				title='<%= messages.getMessage("iterations.tableheading.actions") %>'>


				<xplanner:isUserAuthorized name="iteration"
					permission='<%=action.getPermission()%>'>
					<xplanner:action actionRenderer='<%=action%>'
						targetBean='<%=iteration%>'
						fkey='<%=iteration.getProject().getId()%>' />
			...
			<dt:column
				...
				<xplanner:link page="/do/view/iteration" paramId="oid"
					paramName="iteration" paramProperty="id">
            ${iteration.id}</xplanner:link>
			</dt:column>
			<dt:column sortable="true" sortProperty="name"
				title='<%= messages.getMessage("iterations.tableheading.iteration") %>'>
				<xplanner:link styleClass="name" page="/do/view/iteration"
					paramId="oid" paramName="iteration" paramProperty="id">
            ${iteration.name}</xplanner:link>
			</dt:column>
			<dt:column sortable="true" sortProperty="startDate"
				title='<%= messages.getMessage("iterations.tableheading.startDate") %>'>
				<p>
					<xplanner:formatDate formatKey="format.date" name="iteration"
						property="startDate" />
				</p>
			</dt:column>
			<dt:column sortable="true" sortProperty="endDate"
				title='<%= messages.getMessage("iterations.tableheading.endDate") %>'>
				<p>
					<xplanner:formatDate formatKey="format.date" name="iteration"
						property="endDate" />
				</p>
			</dt:column>
			<dt:column sortable="true" sortProperty="daysWorked"
				title='<%= messages.getMessage("iterations.tableheading.days_worked") %>'
				align="right">
				<p><%= DecimalFormat.format(pageContext, iteration.getDaysWorked()) %></p>
			</dt:column>
			<dt:column
				title='<%= messages.getMessage("iterations.tableheading.stories") %>'
				align="right">
				<p><%= iteration.getUserStories().size() %></p>
			</dt:column>
	...
	<logic:equal name="iterationCount" value="0">
		<p class="highlighted_message">
			<bean:message key="iterations.none" />
		</p>
	</logic:equal>


	<tiles:put name="actions" direct="true">
		<xplanner:actionButtons name="project" id="action">
