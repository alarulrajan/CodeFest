...
<bean:define id="person" name="person" scope="request"
	...
	<tbody>
		<logic:iterate collection='<%= tasks %>' id="currentTask"
			...
			<tr <%= isActive.equals("true") ? "class='active'" : ""%>>
				...
				<td><html:link page="/do/view/task" paramId="oid"
				...
				<td align="center"><logic:equal name="isActive" value="true">
						<bean:message key="person.task.active" />
					</logic:equal></td>
				<td align="center"><xplanner:isUserAuthorized name="task"
						permission="edit" projectId="${iteration.project.id}">
						<xplanner:link styleClass="image" page="/do/edit/time"
							paramId="oid" paramName="task" paramProperty="id"
							fkey='<%= task.getId() %>'>
							<html:img pageKey="edittime.icon" alt="Enter Time" border="0" />
						</xplanner:link>
						<xplanner:link styleClass="image" page="/do/edit/task"
							paramId="oid" paramName="task" paramProperty="id">
