...
<xplanner:content definition="tiles:view">
  ...
  %>
		...
		<xplanner:writableTable
			wholeCollection='<%=iteration.getUserStories() %>'
			permissions="edit,delete" sort="list" defaultsort="7"
			rowDecorator='<%= new TaskRowDecorator() %>' id="objecttable"
			name="iterationTasks" property="iterationTasks"
    ...
    %>
			...
			<dt:column sortable="true" sortProperty="story.orderNo"
				title='<%= messages.getMessage(IterationStoriesPage.STORIES_ORDER_COLUMN) %>'>
      ${story.orderNo}
    </dt:column>
			<dt:column sortable="true" sortProperty="story"
				title='<%= messages.getMessage("person.tableheading.story") %>'
				group="1">
        ...
        ${story.name}
			...
			<dt:column sortable="true" sortProperty="name"
				title='<%= messages.getMessage("person.tableheading.task") %>'>
        ...
        ${task.name}
			...
			<dt:column sortable="true" sortProperty="acceptorId"
    ...
    %>
			<dt:column sortable="true" sortProperty="statusOrder"
				title='<%= messages.getMessage("tasks.tableheading.status") %>'
				align="center">
				<%if(toolTip!=""){ %>
				<html:img page='<%=image%>' />
				<%}else{ %>
			...
			<dt:column sortable="true" sortProperty="estimatedHours"
				title='<%= messages.getMessage("tasks.tableheading.estimate") %>'
				align="right">
				<%= DecimalFormat.format(pageContext, task.getEstimatedHours()) %>
			</dt:column>
			<dt:column sortable="true" sortProperty="actualHours"
				title='<%= messages.getMessage("tasks.tableheading.actual") %>'
				align="right">
				<%= DecimalFormat.format(pageContext, task.getActualHours()) %>
			</dt:column>
			<dt:column sortable="true" sortProperty="remainingHours"
				title='<%= messages.getMessage("tasks.tableheading.remaining") %>'
				align="right">
				<%= task.isCompleted() ? "" : DecimalFormat.format(pageContext, task.getRemainingHours()) %>
			</dt:column>
			<dt:column sortable="true" sortProperty="disposition"
				title='<%= messages.getMessage("tasks.tableheading.disposition") %>'>
				<%= messages.getMessage(task.getDispositionNameKey()) %>
			</dt:column>
			<dt:column sortable="true" property="type"
