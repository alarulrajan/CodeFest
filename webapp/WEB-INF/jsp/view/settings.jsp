...
<bean:define id="messages" name='<%=Globals.MESSAGES_KEY%>' type="org.apache.struts.util.MessageResources"/>
 --%>
	<xplanner:writableTable wholeCollection='${settings}'
		permissions="admin.edit,sysadmin.delete" name="pageScope.settings"
		...
		<dt:column
			...
			<xplanner:link href="setting" paramId="oid" paramName="setting"
				paramProperty="id">
				<bean:write name="setting" property="id" />
			</xplanner:link>
		</dt:column>
		<dt:column
			title='<%= messages.getMessage("projects.tableheading.name") %>'
			sortable="true" sortProperty="name">
			<xplanner:link styleClass="name" href="setting" paramId="oid"
				paramName="setting" paramProperty="id">
				<bean:write name="setting" property="name" />
			</xplanner:link>
		</dt:column>
	...
	<span class="links"> <xplanner:isUserAuthorized
			...
			<xplanner:link page="/do/edit/setting">
				<bean:message key="settings.link.add_setting" />
			</xplanner:link>
