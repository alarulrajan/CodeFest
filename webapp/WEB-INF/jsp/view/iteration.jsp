...
<xplanner:content definition="tiles:view">
	...
	<logic:greaterThan name="storyCount" value="0">
		...
		<form action="<html:rewrite page="/do/edit/reorderstories"/>"
			method="POST" name="reordering">

			<xplanner:isUserAuthorized name="iteration" permission="edit">
				<a class="saveOrder"
					href="javascript:document.forms['reordering'].submit()"><bean:message
						key="iteration.link.save_order" /></a>

				<a class="moveMultiple"
					href="javascript:submitMoveMultiple('<html:rewrite page="/do/move/userstories"/>')">
					<bean:message key="iteration.link.move_multiple" />
				</a>
			</xplanner:isUserAuthorized>

			<input type="hidden" name="iterationId"
				value="<%=iteration.getId()%>" /> <input type="hidden" name="oid"
				value="<%=iteration.getId()%>" />
			<%int index = -1;
...
int defaultSortColumnIndex = (hasIterationEditPermission ? 4 : 2); %>
			...
			<xplanner:writableTable defaultsort="<%=defaultSortColumnIndex %>"
				defaultorder="ascending" name="requestScope.iteration"
				id="objecttable" wholeCollection='<%=iteration.getUserStories() %>'
				permissions="edit,delete" property="userStories"
    ...
    %>
				...
				<xplanner:isUserAuthorized name="iteration" permission="edit">
					<dt:column sortable="false"
						title='<%= messages.getMessage(IterationStoriesPage.STORIES_SELECTED_COLUMN)%>'>
						<input type="checkbox" name="selected[<%=index%>]"
							value='<%=story.getId()%>'>
					</dt:column>
				</xplanner:isUserAuthorized>

				<xplanner:actionButtonsColumn name="story" id="action"
					title='<%= messages.getMessage("stories.tableheading.actions") %>'>
					<%--DEBT: Remove duplication. Everything in the actionButtons should be extracted --%>
					<xplanner:isUserAuthorized name="story"
						...
						<xplanner:link styleClass="image"
							...
							<html:img styleClass="${action.name}"
								page='<%=action.getIconPath()%>' alt='<%=action.getName()%>'
								border="0" />
						</xplanner:link>
					</xplanner:isUserAuthorized>
				...
				<dt:column
					...
					<xplanner:link page="/do/view/userstory" paramId="oid"
						paramName="story" paramProperty="id">
        ${story.id}</xplanner:link>
				</dt:column>

				<dt:column sortable="true" sortProperty="orderNo"
					title='<%= messages.getMessage(IterationStoriesPage.STORIES_ORDER_COLUMN)%>'>
					<c:choose>
						<c:when test="<%=hasIterationEditPermission %>">
							<input type="text" name="orderNo[<%=index%>]"
								value='<%=story.getOrderNo()%>' size="2"
			...
			${story.orderNo}
					...
					<input type="hidden" name="storyId[<%=index%>]"
						value='<%=story.getId()%>'>
				</dt:column>

				<dt:column sortable="true" sortProperty="name"
					title='<%= messages.getMessage("stories.tableheading.name")%>'>
					<xplanner:link page="/do/view/userstory" styleClass="name"
						paramId="oid" paramName="story" paramProperty="id">
        ...
        ${story.priority}</dt:column>
				<dt:column sortable="true" sortProperty="customer.name"
					title='<%= messages.getMessage("stories.tableheading.customer")%>'>
					<logic:notEmpty name="story" property="customer">
						<bean:define id="customer" name="story" property="customer"
							type="net.sf.xplanner.domain.Person" />
						<xplanner:link page="/do/view/person" paramId="oid"
							paramName="customer" paramProperty="id">
					${customer.initials}
                </xplanner:link>
					</logic:notEmpty>
				...
				<dt:column sortable="true" sortProperty="percentCompleted"
					title='<%= messages.getMessage(IterationStoriesPage.STORIES_PROGRESS_COLUMN)%>'>
					<xplanner:progressBar actual='<%= story.getCachedActualHours() %>'
						estimate='<%= visualEstimation %>'
						complete='<%= story.isCompleted() %>' width="100%" />
				</dt:column>
				<%
      if (!"image".equals(new XPlannerProperties().getProperty("xplanner.progressbar.impl"))) { %>
				<dt:column sortable="true"
					sortProperty='<%=UserStory.CACHED_TASK_BASED_ACTUAL_HOURS%>'
					align="right"
				...
				<%}%>
				<dt:column sortable="true"
					sortProperty='<%=UserStory.TASK_BASED_REMAINING_HOURS%>'
					align="right"
            ...
            DecimalFormat.format(pageContext, story.getTaskBasedEstimatedOriginalHours()) %></span>
				...
				<%--    </dt:column>--%>
				<dt:column sortable="true" sortProperty="taskCount" align="right"
					title='<%= messages.getMessage("stories.tableheading.tasks")%>'>
					<span
						class='<%= story.getTasks().size() == 0 ? "highlighted_message" : "" %>'><%= story.getTasks().size() %></span>
				</dt:column>
				<dt:column sortable="true" sortProperty="trackerName" align="center"
					title='<%= messages.getMessage("stories.tableheading.tracker")%>'>
					<logic:notEqual name="story" property="trackerId" value="0">
						...
						<xplanner:link page="/do/view/person" paramId="oid"
							paramName="tracker" paramProperty="id">
				...
				<dt:column align="center" sortable="true"
					title='<%= messages.getMessage("stories.tableheading.status")%>'>
					<%=messages.getMessage(story.getStatusEnum().getNameKey())%>
				</dt:column>
			</xplanner:writableTable>
			<xplanner:isUserAuthorized name="iteration" permission="edit">
				<a class="saveOrderBottom"
					href="javascript:document.forms['reordering'].submit()"><bean:message
						key="iteration.link.save_order" /></a>
			</xplanner:isUserAuthorized>
	</logic:greaterThan>
