...
<xplanner:content>
	...
	<h1>
		<span class="title"> <bean:message key="history" />:
		</span> <span class="object_name"><bean:write name="domainContext"
				property="targetObject.name" /></span>
	</h1>
	<%
...
%>
	...
	<logic:greaterThan name="HistoryCount" value="0">
		<span class="title">
			<h1>
				<bean:message key="history.title" />
			</h1>
		</span>
		<dt:table uid='<%=HistoryPage.EVENT_TABLE_ID%>'
			htmlId='<%=HistoryPage.EVENT_TABLE_ID%>' id="event"
			name='<%=HistoryPage.EVENT_TABLE_ID%>' styleClass="objecttable"
			requestURI="">
           ...
           pageContext.setAttribute("History", History); %>
			...
			<dt:column sortable="true" sortProperty="action"
				title='<%= messages.getMessage("history.tableheading.action") %>'>
				<bean:write name="History" property="action" />
			</dt:column>
			<dt:column
				title='<%= messages.getMessage("history.tableheading.by") %>'>
				<xplanner:link page="/do/view/person" paramId="oid" paramName="who"
					paramProperty="id">
            ${who.name}
          </xplanner:link>
			</dt:column>
			<dt:column sortable="true" sortProperty="whenHappened"
				title='<%= messages.getMessage("history.tableheading.when") %>'>
				<xplanner:formatDate name="History" property="whenHappened" />
			</dt:column>
			<dt:column sortable="true" sortProperty="description"
				title='<%= messages.getMessage("history.tableheading.description") %>'>
...
%>
	...
	<bean:size id="containerEventCount"
		name='<%=HistoryPage.CONTAINER_EVENT_TABLE_ID%>' />
	<logic:greaterThan name="containerEventCount" value="0">
		<span class="title"><h1>
				<bean:message key="history.container.title" />
			</h1></span>
		<dt:table uid='<%=HistoryPage.CONTAINER_EVENT_TABLE_ID%>'
			htmlId='<%=HistoryPage.CONTAINER_EVENT_TABLE_ID%>' id="event"
			name='<%=HistoryPage.CONTAINER_EVENT_TABLE_ID%>'
          ...
          %>
			...
			<dt:column sortable="true" sortProperty="objectType"
				title='<%= messages.getMessage("history.tableheading.type") %>'>
				<bean:write name="History" property="objectType" />
			</dt:column>
			<dt:column
				title='<%= messages.getMessage("history.tableheading.name") %>'>
