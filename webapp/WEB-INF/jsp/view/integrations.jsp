...
<xplanner:content>
	...
	<xplanner:navigation oid='<%= Integer.parseInt(projectId) %>'
		type="net.sf.xplanner.domain.Project" />
	...
	<db:useBeans id="currentIntegrations"
		where="state = ? and projectId = ?"
		type="com.technoetic.xplanner.domain.Integration">
		<db:useBeansParameter value='<%= new Character(Integration.ACTIVE) %>'
			type="char" />
		<db:useBeansParameter value='<%= projectId %>' type="string" />
	</db:useBeans>

	<db:useBeans id="pendingIntegrations"
		where="state = ? and projectId = ?"
		type="com.technoetic.xplanner.domain.Integration">
		<db:useBeansParameter
			value='<%= new Character(Integration.PENDING) %>' type="char" />
		<db:useBeansParameter value='<%= projectId %>' type="string" />
	</db:useBeans>
...
%>
		...
		<logic:present name="currentIntegration">
			<table cellpadding="1" class="description">
				<tr>
					<td><db:useBean id="person"
							type="net.sf.xplanner.domain.Person"
							oid='<%= new Integer(currentIntegration.getPersonId()) %>' />
						<p>
							<strong><bean:message
									key="integrations.current_integrator" />:</strong>
							<xplanner:link page="/do/view/person" paramId="oid"
								paramName="person" paramProperty="id">
								<bean:write name="person" property="name" />
							</xplanner:link>
							<br> <strong><bean:message
									key="integrations.started_at" />:</strong>
							<xplanner:formatDate formatKey="format.datetime"
								name="currentIntegration" property="whenStarted" />
						</p>
						<p>
							<bean:write name="currentIntegration" property="comment" />
						</p>
						<p>
							<xplanner:isUserAuthorized resourceType="system.project"
					...
					<td valign="top">
						<p>
							<img src="<html:rewrite page="/images/madhack.gif" />" width="48"
								height="48">
						</p>
					</td>
				</tr>
		...
		<logic:notPresent name="currentIntegration">
			<table class="description">
				<tr>
					<td><strong>No current integration.</strong></td>
				</tr>
			</table>
		</logic:notPresent>
		<br />
		...
		<p>
			<strong><bean:message key="integrations.waiting_line" />:</strong>
		</p>
		<% } %>
		...
		<table border="0" cellpadding="1" cellspacing="1" class="objecttable">
			...
			<logic:iterate id="integration"
				collection='<%= pendingIntegrations %>' indexId="n"
				type="com.technoetic.xplanner.domain.Integration">
				<db:useBean id="person" type="net.sf.xplanner.domain.Person"
					oid='<%= new Integer(integration.getPersonId()) %>' />
				<tr>
					<td align="center"><xplanner:link page="/do/view/person"
							paramId="oid" paramName="person" paramProperty="id">
							<bean:write name="person" property="name" />
						</xplanner:link></td>
					<td align="center"><xplanner:formatDate
							formatKey="format.datetime" name="integration"
							property="whenRequested" /></td>
					<td><bean:write name="integration" property="comment" /></td>
					<xplanner:isUserAuthorized resourceType="system.project"
						permission="integrate">
						<td bgcolor="#f0f0f0" align="center">
							<% if (n.equals(new Integer(0)) && currentIntegrations.size() == 0) { %>
							<input type="submit" name="action.start" value="Start" /> <% } %> <input
							type="submit"
		...
		<xplanner:isUserAuthorized resourceType="system.project"
			...
			<table border="0" cellpadding="1" cellspacing="1" class="objecttable">
				<tr>
					<td bgcolor="#f0f0f0" align="center"><xplanner:authenticatedUser
							id="user" /> <%
              ...
              %> <bean:message key="integrations.tableheading.who" />: <html:select
							name="defaultIntegration" property="personId">
							<html:option value="0">&nbsp;</html:option>
							<xplanner:personOptions />
						</html:select> <bean:message key="integrations.tableheading.what" />: <input
						type="text" name="comment" size="40" /> <input type="submit"
						name="action.join"
		...
		<db:useBeans id="recentIntegrations"
			type="com.technoetic.xplanner.domain.Integration"
			where="state in (?, ?) and projectId = ?" order="when_complete desc"
			size="20">
			<db:useBeansParameter
				value='<%= new Character(Integration.FINISHED) %>' type="char" />
			<db:useBeansParameter
				value='<%= new Character(Integration.CANCELED) %>' type="char" />
			<db:useBeansParameter value='<%= projectId %>' type="string" />
		</db:useBeans>


		<p>
			<strong><bean:message key="integrations.recent_integrations" />:</strong>
		</p>
		...
		<logic:greaterThan name="recentIntegrationCount" value="0">
			...
			<dt:table id="objecttable" list='<%= recentIntegrations %>'
				border="0" styleClass="objecttable" requestURI=""
				defaultorder="descending" defaultsort="3" length="20">
				<%--    <dt:setProperty name="paging.banner.placement" value="bottom" />--%>
				<bean:define id="integration" name="objecttable"
					type="com.technoetic.xplanner.domain.Integration" />
				<dt:column sortable="true"
					title='<%= messages.getMessage("integrations.tableheading.who")%>'>
					<db:useBean id="person" type="net.sf.xplanner.domain.Person"
						oid='<%= new Integer(integration.getPersonId()) %>' />
					<xplanner:link page="/do/view/person" paramId="oid"
						paramName="person" paramProperty="id">
				...
				<dt:column sortable="true" align="center"
					title='<%= messages.getMessage("integrations.tableheading.start")%>'>
					<xplanner:formatDate formatKey="format.datetime" name="integration"
						property="whenStarted" />
				</dt:column>
				<dt:column sortable="true" align="center"
					title='<%= messages.getMessage("integrations.tableheading.finish")%>'>
					<xplanner:formatDate formatKey="format.datetime" name="integration"
						property="whenComplete" />
				</dt:column>
				<dt:column sortable="true" align="center"
					title='<%= messages.getMessage("integrations.tableheading.duration")%>'>
					<%= DecimalFormat.format(pageContext, integration.getDuration()) %>
				</dt:column>
				<dt:column sortable="true" align="center"
					title='<%= messages.getMessage("integrations.tableheading.state")%>'>
				...
				<dt:column sortable="true"
					title='<%= messages.getMessage("integrations.tableheading.what")%>'>
