...
<bean:define id="domainContext" name="domainContext"
	type="com.technoetic.xplanner.tags.DomainContext" />
<%
...
<table class="noteSectionHeader">
	<tr>
		<td><span class="noteSectionLabel"><bean:message
					key="notes.label.notes" /></span></td>
		<td nowrap>
			<div align="right">
				<% if (isNoteEditor) { %>
				<span class="noteSectionLinks"> <xplanner:link
						page="/do/edit/note" paramId="attachedToId" paramName="oid">
...
<logic:greaterThan name="notesCount" value="0">
	<logic:iterate name="notes" scope="page" id="note" indexId="n"
		...
		<table class="notesTable">
					...
					<div>
						<xplanner:isUserAuthorized name="note" permission="edit"
							...
							<xplanner:link page="/do/edit/note">
								...
								<xplanner:linkParam id="attachedToId" name="note"
									property="attachedToId" />
							</xplanner:link>
							<xplanner:link page="/do/delete/note"
								onclick='<%="return confirm(\'Do you want to delete note " + StringUtilities.replaceQuotationMarks(StringEscapeUtils.escapeJavaScript(note.getSubject())) + "?\')"%>'>
					...
					<div>
						<ul>
							<li><strong><bean:message key="notes.label.subject" /></strong>
								${note.subject}</li>
							<li><db:useBean id="author"
									type="net.sf.xplanner.domain.Person"
									oid='<%= new Integer(note.getAuthorId()) %>' /> <strong><bean:message
										key="notes.label.author" /> </strong> <html:link page="/do/view/person"
									paramId="oid" paramName="author" paramProperty="id">
                    ${author.name}</html:link></li>
							<li><strong><bean:message key="notes.label.date" />
							</strong> <xplanner:formatDate formatKey="format.datetime" name="note"
									property="submissionTime" /></li>
							<logic:present name="note" property="file">
								<logic:notEqual name="note" property="file.name" value="">
									<li><strong><bean:message
												key="notes.label.attachment" /> </strong> <html:link
											page="/do/download/attachment" paramId="oid" paramName="note"
											paramProperty="id">
											<bean:write name="note" property="file.name" />
										</html:link></li>
									<li><strong><bean:message
												key="notes.label.attachment.count" /> </strong> <%= note.getAttachmentCount() %>
									</li>
								</logic:notEqual>
							</logic:present>
						</ul>
			...
			<tr>
				<td class="noteBody"><div>
						<xplanner:twiki name="note" property="body" />
					</div></td>
			</tr>
		</table>
	</logic:iterate>
