...
<xplanner:content>
	...
	<xplanner:isUserAuthorized permission="admin.hide"
		resourceType="system.person">
		<db:useBeans id="people" type="net.sf.xplanner.domain.Person"
			order="is_hidden,name asc" />
	</xplanner:isUserAuthorized>
	<xplanner:isUserAuthorized permission="admin.hide" negate="true"
		resourceType="system.person">
		<db:useBeans id="people" type="net.sf.xplanner.domain.Person"
	...
	<logic:greaterThan name="projectId" value="0">
		...
		<p class="title">
			<bean:message key="people.project.title"
...
%>
		...
		<logic:greaterThan name="peopleCount" value="0">
			...
			<xplanner:writableTable name="peopleToShow" id="objecttable"
				styleClass="objecttable" requestURI="" defaultsort="3"
  ...
  %>
				<%--               <%
                 ...
                 %>
--%>
				<xplanner:actionButtonsColumn id="action" name="person"
					title='<%= messages.getMessage("people.tableheading.actions") %>'>
					<%
...
%>
				...
				<dt:column sortable="true" sortProperty="userId"
					title='<%= messages.getMessage("people.tableheading.userid") %>'>
    ...
    ${person.userId}</xplanner:link>
				...
				<dt:column sortable="true" sortProperty="name"
					title='<%= messages.getMessage("people.tableheading.name") %>'>
					<xplanner:link page="/do/view/person" paramId="oid"
						paramName="person" paramProperty="id">
    ${person.name}</xplanner:link>
				</dt:column>
				<dt:column sortable="true" sortProperty="initials"
					title='<%= messages.getMessage("people.tableheading.initials") %>'>
					<p>${person.initials}</p>
				</dt:column>
				<dt:column
					title='<%= messages.getMessage("people.tableheading.phone") %>'>
					<p>${person.phone}</p>
				</dt:column>
				<dt:column sortable="true" sortProperty="email"
					title='<%= messages.getMessage("people.tableheading.email") %>'>
	...
	<span class="links"> <xplanner:isUserAuthorized
			resourceType="system.person" permission="create.person">
			<xplanner:link page="/do/edit/person" includeProjectId="false">
				<bean:message key="people.link.add_person" />
			</xplanner:link>
			<xplanner:link page="/do/import/people" includeProjectId="false">
